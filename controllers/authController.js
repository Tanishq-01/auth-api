...
const {signupSchema,signinSchema,acceptCodeSchema, changePasswordSchema, acceptFPSchema} = require('../middlewares/validator.js');
...
exports.verifyForgotPasswordCode = async (req,res)=>{
  const {email,providedCode,password} = req.body;
  try{
    const {error,value} = acceptFPSchema.validate({email,providedCode,password});
    if(error){
      return res
      .status(401)
      .json({success:false,message:error.details[0].message});
    ...
    if(hashedCodeValue == existingUser.forgotPasswordCode){
      ...
      return res
      .status(200)
      .json({success:true,message:"Your Password is Updated!"});
    }
